...
def recommend_portfolio(intent_request):
    ...
    if int(age) >= 0 and int(age) <= 65:
        if int(investment_amount) >= 5000:
            if risk_level == "none":
                reccomendation = "100% in Bonds (AGG) and 0% in equites (SPY)"
            elif risk_level == "low":
                reccomendation = "60% in Bonds (AGG) and 40% in equites (SPY)"
            elif risk_level == "medium":
                reccomendation = "40% in Bonds (AGG) and 60% in equites (SPY)"
            elif risk_level == "high":
                reccomendation = "20% in Bonds (AGG) and 80% in equites (SPY)"
            else:
                reccomendation = "Sorry I didn't quite catch that"
